"""
指南模块 6 — NexusAgent 企业培训配套文档代码化

章节概述
--------
本模块对应培训课程第 3 天的扩展阅读材料。
内容涵盖：Python 基础、大模型 API、Prompt 工程、RAG、Agent、微调与部署。

使用方式
--------
>>> from nexus_agent.docs_generated.guide_0006 import get_summary
>>> print(get_summary())
...
def get_summary() -> str:
    """返回本章摘要"""
    return f"Guide 6: " + " | ".join(SECTIONS[:3])
...
def explain_concept(name: str) -> str:
    ...
    return explanations.get(name, f"概念 {name} 详见课件 Day 3")
